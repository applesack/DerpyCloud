...
interface LockSystem {
     ...
     *
     * - 返回一个元组, 第一个元素是一个回调, 第二个元素是操作状态; 如果操作状态为[Locks.Errors.None],
     * 那么调用第一个元素将释放之前持有的锁; 如果操作状态为其他状态, 那么返回的回调无效, 并不会持有锁;
     * 在WebDAV的意义上, 调用释放不会解锁资源, 但是一旦确认声明操作成功, 在该锁被释放之前无法再次确认;
     *
     * - 如果[confirm]调用返回[Locks.Errors.ConfirmationFailed], 则处理程序将继续尝试使用其他的锁;
     ...
     * @param name1      资源名2
     */
    fun confirm(conditions: List<Locks.Condition>, name0: String, name1: String = ""): Pair<() -> Unit, Locks.Errors>

    /**
     * **使用提供的信息(深度, 持续时间, 所有者, 资源名)去创建一个锁**,
     * 其中锁的深度只有两个取值, 0 或者 infinite;
     *
     * - 如果该调用返回了[Locks.Errors.Locked], 响应应该写入"423 Locked"状态码,
     ...
     * **使用给定的token刷新锁**
     *
     * - 如果[refresh]调用返回了[Locks.Errors.Locked], 响应应该写入"423 Locked"状态码;
     * 如果调用返回了[Locks.Errors.NoSuchLock], 响应应该写入"412 Precondition Failed"状态码;
     * 如果该调用抛出异常, 那么响应应该写入"500 Internal Server Error"状态码
     *
     * 参考 http://www.webdav.org/specs/rfc4918.html#rfc.section.9.10.6
     */
     ...
     * **使用指定的token解锁**
     *
     * - 如果[unlock]调用返回[Locks.Errors.Forbidden], 响应应该写入"403 Forbidden"状态码;
     * 如果调用返回[Locks.Errors.Locked], 响应应该写入"423 Locked"状态码;
     * 如果调用返回[Locks.Errors.ConfirmationFailed], 响应应该写入"409 Conflict"状态码;
     * 如果该调用抛出异常, 那么响应应该写入"500 Internal Server Error"状态码
     *
